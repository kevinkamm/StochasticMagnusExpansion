clear;close all 
gpuDevice(1);
% rng('default')%set global seed to 0 for comparison if wanted, any positive 
% integer is possible too
%% Parameters
%% 
...
%% Initialize
[time,BM,ind_t,ind_p]=B0_fix_param_initialize(param);
%% Calculate methods
Result=B0_fix_run(time,BM,ind_t,ind_p,methods,'loop',loop);
%% Calculate errors
if exist('errors')==1 && ~isempty(errors)
%     Errors=B0_fix_errors(Result,time,errors,'p',1,'T',param.T);
    Errors=B0_fix_errors(Result,time,errors);
end
if exist('errors_total')==1 && ~isempty(errors_total)
    Errors.total=B0_fix_errors_total(Result,param,time,errors_total,'percentile',.99);
end
%% Plot 2D Figures
if exist('plots')==1 && ~isempty(plots)
    Plots=B0_fix_plot(Result,time,plots,'show_title',0,'blackwhite','false');
end
if exist('error_plots')==1 && ~isempty(error_plots)
    Error_Plots=B0_fix_plot_err(Errors,error_plots,'show_title',0,'percentile',.99,'blackwhite','false');
end
%% Plot 3D Figures
if exist('surfaces')==1 && ~isempty(surfaces)
    Surfaces=B0_fix_surface(Result,time,surfaces,'show_title',0);
end
%% Create output
if exist('output')==1 && ~isempty(output)
    ...
    B0_fix_output(param,Result,output,vararg{:});
end
disp('done');