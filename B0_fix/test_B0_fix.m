...
%% Calculate methods
Result=B0_fix_run(time,BM,ind_t,ind_p,methods,'Comp Device','cpu');
%% Calculate errors
...
if exist('plots')==1 && ~isempty(plots)
    Plots=B0_fix_plot(Result,time,plots,'show_title',0);
end
if exist('error_plots')==1 && ~isempty(error_plots)
    Error_Plots=B0_fix_plot_err(Errors,error_plots,'show_title',0,'percentile',.99);
end
%% Plot 3D Figures
