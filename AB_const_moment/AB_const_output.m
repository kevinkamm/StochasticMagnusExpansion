...
function AB_const_output_pdf(param,A,B,Result,varargin)
    ...
    fprintf(in_file,...
        '\t\\input{%s}\n',changeSlash(filePath));
    fprintf(in_file,...
        '\\subsection{Values of Moment}\n');
    filePath=pdf_moment_values();
    fprintf(in_file,...
        '\t\\input{%s}\n',changeSlash(filePath));
...
%     delFile(copyPath);
    ...
    function filePath=pdf_moment_values()
        filePath=[root,'\','temp','\','momemt_values','.','tex'];
        delFile(filePath);
        file = fopen(filePath,'a');
        methods=fieldnames(Result);
        fprintf(file,...
...
%                 methods{k});
%             fprintf(file,...
%                 '%g $\\left[%1.3g,%1.3g\\right]$ & ',...
%                 Result.(methods{k}).fm.m(1,1),...
%                 Result.(methods{k}).fm.ci(1,1,1),...
%                 Result.(methods{k}).fm.ci(1,1,2)...
%                 );
%             fprintf(file,...
%                 '%g $\\left[%1.3g,%1.3g\\right]$ & ',...
%                 Result.(methods{k}).fm.m(1,2),...
%                 Result.(methods{k}).fm.ci(1,2,1),...
%                 Result.(methods{k}).fm.ci(1,2,2)...
%                 );
%             fprintf(file,...
%                 '%g $\\left[%1.3g,%1.3g\\right]$ & ',...
%                 Result.(methods{k}).fm.m(2,1),...
%                 Result.(methods{k}).fm.ci(2,1,1),...
%                 Result.(methods{k}).fm.ci(2,1,2)...
%                 );
%             fprintf(file,...
%                 '%g $\\left[%1.3g,%1.3g\\right]$ \\\\\n  ',...
%                 Result.(methods{k}).fm.m(2,2),...
%                 Result.(methods{k}).fm.ci(2,2,1),...
%                 Result.(methods{k}).fm.ci(2,2,2)...
%                 );
            fprintf(file,...
                '\\text{%s} & ',...
                methods{k});
            fprintf(file,...
                '%g $\\pm %1.3g$ & ',...
                Result.(methods{k}).fm.m(1,1),...
                Result.(methods{k}).fm.ci(1,1)...
                );
            fprintf(file,...
                '%g $\\pm %1.3g$ & ',...
                Result.(methods{k}).fm.m(1,2),...
                Result.(methods{k}).fm.ci(1,2)...
                );
            fprintf(file,...
                '%g $\\pm %1.3g$ & ',...
                Result.(methods{k}).fm.m(2,1),...
                Result.(methods{k}).fm.ci(2,1)...
                );
            fprintf(file,...
                '%g $\\pm %1.3g$ \\\\\n  ',...
                Result.(methods{k}).fm.m(2,2),...
                Result.(methods{k}).fm.ci(2,2)...
                );
        end
        fprintf(file,...
            '\\end{tabular}\n');
