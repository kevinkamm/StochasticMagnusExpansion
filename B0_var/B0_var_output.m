function B0_var_output(exm,param,A,Result,output,varargin)
...
%
    for kk=1:1:length(output)
        switch output{kk}
            case 'pdf'
                B0_var_pdf(exm,param,A,Result,varargin)
            otherwise
                disp('Output type unknown.');
        end
    end
end
function B0_var_pdf(exm,param,A,Result,varargin)
    pic_type='eps';
    save_param='epsc';
    root=[pwd,'\','Pdf'];
    fileName=...
            sprintf('B0_var_T%1.3g_d%i_N%i_M%i',...
...
%     delFile(copyPath);
    ...
    function filePath=pdf_Model()
        ...
        fprintf(file,...
            str);
        if strcmp(exm,'scaled')
            fprintf(file,...
                'Here we chose:\n');
            str=...
            ['\\begin{align*}\n'...
            'A_t=\\frac{\\tilde{A}_t}{\\sigma(\\tilde{A}_t)},'...
            '\\end{align*}\n'];
            fprintf(file,...
                str);
            fprintf(file,...
