...
%% Calculate methods
Result=B0_var_run(time,BM,ind_t,ind_p,A,methods);
%% Calculate errors
...
if exist('plots')==1 && ~isempty(plots)
    Plots=B0_var_plot(Result,time,plots,'show_title',0,'resolution',1000);
end
if exist('error_plots')==1 && ~isempty(error_plots)
    Error_Plots=B0_var_plot_err(Errors,error_plots,'show_title',0,'percentile',.99);
end
%% Plot 3D Figures
