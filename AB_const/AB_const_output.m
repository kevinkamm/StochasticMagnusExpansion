function AB_const_output(param,A,B,Result,output,varargin)
...
%
    for kk=1:1:length(output)
        switch output{kk}
            case 'pdf'
                AB_const_output_pdf(param,A,B,Result,varargin)
            otherwise
                disp('Output type unknown.');
        end
    end
end
function AB_const_output_pdf(param,A,B,Result,varargin)
    pic_type='eps';
    save_param='epsc';
    root=[pwd,'\','Pdf'];
    fileName=...
            sprintf('AB_const_T%1.3g_d%i_N%i_Neuler%i_M%i',...
            param.T,param.d,param.N,param.N_fine,param.M);
    tempPath=[root,'\','temp'];
    if exist(tempPath)==7
        rmdir(tempPath,'s');
    end
...
%     delFile(copyPath);
    ...
    function filePath=pdf_Model()
        ...
        fprintf(file,...
            '\\end{align*}\n');
        fprintf(file,...
            'The spectral norm of $A$ is $%g$.\n',norm(A,2));
        fprintf(file,...
            'and the spectral norm of $B$ is $%g$.\n',norm(B,2));
        fclose(file);
    end
    ...
    function filePath=pdf_comp_times()
        ...
        fprintf(file,...
            '\\toprule\n');
        if size(Result.(methods{1}).ctime.total,2)>1
            for iloop=1:1:size(Result.(methods{1}).ctime.total,2)
                fprintf(file,...
                        '\\multicolumn{4}{c}{Run %d}\\\\\n ',iloop);
                fprintf(file,...
                        '\\midrule\n ');
                for k=1:1:length(methods)
                    ...
                    if isfield(Result.(methods{k}).ctime,'log')
                        fprintf(file,...
                        '%g & ',...
                        Result.(methods{k}).ctime.log(iloop));
                    else
                        fprintf(file,...
                            '0 & ');
                    end
                    if isfield(Result.(methods{k}).ctime,'expm')
                        fprintf(file,...
                        '%g & ',...
                        Result.(methods{k}).ctime.expm(iloop));
                    else
                        fprintf(file,...
                            '0 & ');
                    end
                    fprintf(file,...
                        '%g \\\\\n',...
                        Result.(methods{k}).ctime.total(iloop));
                end
            end
                fprintf(file,...
                ...
                for k=1:1:length(methods)
                    ...
                    if isfield(Result.(methods{k}).ctime,'log')
                        fprintf(file,...
                        '%g & ',...
                        mean(Result.(methods{k}).ctime.log(:)));
                    else
                        fprintf(file,...
                            '0 & ');
                    end
                    if isfield(Result.(methods{k}).ctime,'expm')
                        fprintf(file,...
                        '%g & ',...
                        mean(Result.(methods{k}).ctime.expm(:)));
                    else
                        fprintf(file,...
                            '0 & ');
                    end
                    fprintf(file,...
                        '%g \\\\\n',...
                        mean(Result.(methods{k}).ctime.total(:)));
                end
        else
            for k=1:1:length(methods)
                fprintf(file,...
