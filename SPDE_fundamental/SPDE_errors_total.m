function [Total,varargout]=SPDE_errors_total(Result,param,time,rows,errors_total,varargin)
...
for k=1:1:length(errors_total)
    ...
    for i=1:1:length(methods)
        ...
        Err_t=err_t(X_ref,X);
        Total.(reference).(methods{i}).Err=Err_t;
...
%             Total.(reference).(methods{i}).ECDFhist=ECDFhist;
    ...
    function Err_t=err_t(X_ref,X)
        Err_t=zeros(1,size(X_ref,4));
            for jj=1:1:size(X_ref,4)
                Err_t(1,jj)=...
                    (norm(X_ref(rows,:,end,jj)-X(rows,:,end,jj),norm_type)./...
                    norm(X_ref(rows,:,end,jj),norm_type));
            end
    end
